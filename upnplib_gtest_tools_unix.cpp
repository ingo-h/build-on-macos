...
// With constructing the object you get a loopback device by default.
{
    ...
    //-------------------
    // set network address
    std::cout << "DEBUG: entered CIfaddr4 constructor\n";
    ...
    std::cout << "DEBUG: finished CIfaddr4 constructor with "
              << inet_ntoa(m_ifa_addr.sin_addr) << "\n";
}
...
{
    std::cout << "DEBUG: entered CIfaddr4::set()\n";
    ...
    // std::cout << "DEBUG! set ifa_name: " << m_ifaddr.ifa_name << ", address:
    // '" << address << "', bitmask: '" << bitmask << "', netmask: " <<
...
bool CIfaddr4Container::add(std::string_view a_Ifname,
    ...
    if (m_ifaddr4Container.empty()) {
        std::cout << "DEBUG: m_ifaddr4Container.push_back(CIfaddr4())\n";
        // On an empty container just push an interface object to it
        m_ifaddr4Container.push_back(CIfaddr4());

        ifaddrs* ifaddr4Dbg = m_ifaddr4Container.at(0).get();
        sockaddr_in* ifaddr = (sockaddr_in*)ifaddr4Dbg->ifa_addr;
        std::cout
            << "DEBUG: called after m_ifaddr4Container.push_back(CIfaddr4()) "
...
ifaddrs* CIfaddr4Container::get_ifaddr(long unsigned int pIdx) {
    ...
    ifaddrs* ifaddr = m_ifaddr4Container.at(pIdx - 1).get();
    sockaddr_in* ifaddr4_in = (sockaddr_in*)ifaddr->ifa_addr;
    std::cout << "DEBUG: called on CIfaddr4Container::get_ifaddr() "
              << inet_ntoa(ifaddr4_in->sin_addr) << "\n";
    std::cout << "DEBUG: called on CIfaddr4Container::get_ifaddr(): ifaddr =  "
              << ifaddr << "\n";
